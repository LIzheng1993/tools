<?php
namespace app\strategy\service;
/**
 * 转换成农历日期
 *author:Lizheng
 *date:2018-09-20
...
class Calendar{
    ...
    private $curData=null;//当前阳历时间  2018-8-2
    private $ylYeal=0;
    private $ylMonth=0;
    private $yldate=0;
    ...
    public function __construct($curData=null){
        if(!empty($curData)){
            $this->curData=$curData;        //设置当前时间 例：2019-1-9
        }else{
            $this->curData=date('Y-n-j');   //设置当前时间为程序运行的时间
        }
        $this->init();						//初始化
    }

    public function init(){
        $basedate='1900-1-31';  //参照日期，此天是正月初一
        $timezone='PRC';		//中国时区
        ...
        for($i=1900; $i<2050 && $offset>0; $i++){
            $temp = $this->getYearDays($i); //计算i年有多少天(农历)
            $offset -= $temp ;
            $this->difmonth+=12;
            //判断该年否存在闰月
